...
export default function BrandsManager({ onMessage }) {
  ...
  const fetchBrands = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        onMessage(data.message || "Failed to load brands", "error");
      }
    } catch {
      onMessage("Failed to load brands", "error");
    } finally {
      setLoading(false);
    }
  }, [search, onMessage]);
  ...
  const handleSave = async () => {
    if (!form.name.trim()) {
      onMessage("Brand name is required", "error");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        onMessage(data.message || (editingBrand ? "Brand updated" : "Brand created"));
        closeDialog();
        fetchBrands();
      } else {
        onMessage(data.message || "Failed to save brand", "error");
      }
    } catch {
      onMessage("Failed to save brand", "error");
    } finally {
      setSaving(false);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (data.success) {
        onMessage(data.message || "Brand deleted");
        setDeleteDialogOpen(false);
        setBrandToDelete(null);
        fetchBrands();
      } else {
        onMessage(data.message || "Failed to delete brand", "error");
      }
    } catch {
      onMessage("Failed to delete brand", "error");
    } finally {
      setDeleting(false);
    }
  };
    ...
    <Box>
      ...
      >
        <Typography sx={{ color: textMuted, fontSize: "0.9rem" }}>
          Shoe makers in your catalog — Nike, Adidas, and more.
        </Typography>
        <Button
          variant="contained"
          startIcon={<Add />}
      ...
      >
        ...
        <DialogContent>
          <Typography sx={{ color: textMuted }}>
            Delete <strong>{brandToDelete?.name}</strong>? This cannot be undone.
          </Typography>
        </DialogContent>
