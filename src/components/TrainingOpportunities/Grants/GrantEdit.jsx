...
import { ArrowBack, Save, CloudUpload, Close as CloseIcon, AttachMoney, Add } from "@mui/icons-material";
...
const GrantEdit = () => {
  ...
  const fetchGrant = async () => {
    try {
      ...
      const res = await fetch(`/api/grants/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      ...
      setForm({
        ...
        deadlineText: grant.deadlineText || "",
        isRolling: grant.isRolling || false,
        imageAltText: grant.imageAltText || "",
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (result.success) {
        await Swal.fire({ title: "Success!", text: "Grant updated successfully.", icon: "success", confirmButtonColor: "#f59e0b" });
        navigate("/marketplace");
      } else {
        throw new Error(result.message || "Failed to update grant");
      }
    } catch (err) {
      setError(err.message || "Failed to update grant");
      await Swal.fire({ title: "Error!", text: err.message || "Failed to update grant", icon: "error", confirmButtonColor: "#f59e0b" });
    } finally {
      setSaving(false);
    }
  };
          ...
          <CardContent sx={{ width: "100%", maxWidth: "100%", boxSizing: "border-box" }}>
            <Stack spacing={3} sx={{ width: "100%" }}>
              ...
              <Box display="flex" gap={2}>
                <Button variant="contained" size="large" startIcon={saving ? <CircularProgress size={20} color="inherit" /> : <Save />} onClick={handleUpdate} disabled={!isFormValid() || saving} sx={{ flex: 1, background: "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)", color: "white", "&:hover": { background: "linear-gradient(135deg, #d97706 0%, #b45309 100%)" }, "&:disabled": { background: "#e0e0e0", color: "#999" } }}>
                  {saving ? "Updating..." : "Update Grant"}
                </Button>
